...
\title{GRIFFIN 1.1: Manual}
\author[a]{Lisong Chen}
\author[b]{Ayres Freitas}
\affil[a]{\small Institute for Theoretical Particle Physics (TTP), Karlsruhe Institute of Technology (KIT), \mbox{76131 Karlsruhe}, Germany}
\affil[b]{\small Pittsburgh Particle-physics Astro-physics \& Cosmology Center
...
\paragraph*{Program release:} GRIFFIN version 1.1 (September xx, 2024).
\vspace{-.5ex}
\paragraph*{Language:} C++11 (2011 ISO C++), for example {\tt gcc 4.8.1} and beyond.
\vspace{-.5ex}
\paragraph*{Download:} \url{http://github.com/lisongc/GRIFFIN/releases/tag/v1.1.0}

\section*{Changes with respect to version 1.0}

\begin{itemize}
\item Implementation of fermion scattering for identical initial- and final-state fermions\newline (\emph{e.g.} Bhabha scattering).
\item Implementation of ${\cal O}(\alpha_f\as)$ corrections in off-resonant contributions to fermion scattering, where $\alpha_f$ denotes electroweak (EW) corrections with closed fermion loops.
\item Correction of bug in {\tt D0.cc} that affects b-quark initial states.
\item More lightweight implementation of dilogarithm, leading to smaller code sizes.
\item Adoption of {\tt cmake} build system; possibility to build a GRIFFIN link library; use of {\tt namespace}.
\end{itemize}
GRIFFIN 1.1 in downward compatible with GRIFFIN 1.0, except for the items noted below with ``Version change''.
...
\medskip\noindent
\fcolorbox{black}{gray!10}{\begin{minipage}{16.2cm}
\paragraph*{Version change:} In GRIFFIN 1.0, $M_{ij}^{\rm offZ}$ was defined as the difference $M_{ij}-M_{ij}^{{\rm exp},s_0}$, \emph{i.e.} the higher-order terms $S$ and $S'$ were excluded from $M_{ij}^{\rm offZ}$ and added separately. The full matrix element $M_{ij}$ is nevertheless the same in versions 1.0 and 1.1.
\end{minipage}}
\bigskip
...
The theory framework is implemented within a structure of classes in C++. In {\sc v1.1}, the SM predictions for EWPOs, the muon decay process, and polarized matrix elements near the Z-peak up to full NNLO and partial NNNLO have been implemented. Yet in principle the {\sc GRIFFIN} framework can also accommodate predictions for alternative observables, BSM models up to arbitrary higher orders. This section will document the structure of the classes by introducing each class and where to find and modify them in the code.
...
\end{tikzpicture}
 \caption{The hierarchy of classes for {\sc GRIFFIN}'s output. The base class \texttt{psobs} is an abstract class where several virtual member functions are defined. The user can in principle define new derived classes based off their own purpose. In {\sc GRIFFIN 1.1}, three types of derived classes, and their sub-derived classes of higher order are defined accordingly. }
    \label{fig:psobsdiag}
\end{figure}
...
\begin{itemize}
    ...
    \item {\textbf{Dependencies}}: \texttt{classes.cc, EWPOZ.cc, B0.cc, C0.cc, D0.cc, ff.cc, li.cc,\linebreak delrho.cc, linex.cc}.
...
\bigskip\noindent
\fcolorbox{black}{gray!10}{\begin{minipage}{16.2cm}
\paragraph*{Version change:} In GRIFFIN 1.0, \texttt{resoffZ()} was defined as the remainder beyond the $R,S,S'$ terms in the complex-pole expansion, whereas in the current version 1.1, it is the remainder beyond the leading pole term $R$ only, see eq.~\eqref{MoffZ}. The combined result from \texttt{result()} is the same in versions 1.0 and 1.1.
\end{minipage}}
\medskip
...
\subsection{Derived class \texttt{mat\_SMaas}}
\begin{itemize}
    \item {\textbf{Inherit from}}: \texttt{matel\_SMNNLO}
    \item {\textbf{Location}}: \texttt{xscaas.h, xscaas.cc}.
    \item {\textbf{Dependencies}}: \texttt{classes.cc, xscnnlo.cc, linex.cc}
    \item {\textbf{Member Functions}}: \texttt{result, coeffR, resoffZ, resoffZaas}
\end{itemize}
This class provides a description of the matrix element $M_{ij}$ that is the same accuracy on the Z peak as \texttt{mat\_SMNNLO}, whereas it is NLO plus $\OO(\alpha_f\as)$ off-peak. For this purpose, $\OO(\alpha_f\as)$ corrections have been incorporated into the \emph{imaginary part} of the leading pole coefficient $R_{ij}$ [in \texttt{coeffR()}] and into the remainder $M^{\rm offZ}_{ij}$ [in \texttt{resoffZ()}]. \texttt{resoffZaas()} returns only the $\OO(\alpha_f\as)$ contribution of the latter. The real part of $R_{ij}$ receives  $\OO(\alpha_f\as)$ corrections via the form factors $\seff{f}$ and $F_A^f$.
...
\mt/\MZ = 1.70...2.14, \\
\sqrt{s}/\mt &= 0.1...6.48. \label{eq:srange1}
\end{align}
...
\mt = 155...195\gev, \\
\sqrt{s} &= 20...1000\gev. \label{eq:srange2}
\end{align}
The ranges \eqref{eq:srange1} and \eqref{eq:srange2} are only relevant for the \texttt{mat\_SMaas} class.
...
GRIFFIN is a collection of modules that can used by either directly including the relevant source files in a user project, or by first compiling GRIFFIN into a library. To build the GRIFFIN library, a C++11 compatible compiler and the \texttt{cmake} build system should be available. Then execute the following commands in the main GRIFFIN directory:
\begin{lstlisting}[language=make]
cmake .
cmake --build .
\end{lstlisting}
\vspace{-2em}
The resulting library is named \texttt{libgriffin.a} on Linux systems.
...
\noindent
\fcolorbox{black}{gray!10}{\begin{minipage}{16.2cm}
\paragraph*{Version change:} In GRIFFIN 1.1, all classes and functions are encoded in the namespace \texttt{griffin}. To access any GRIFFIN elements in a user project one thus has to either add the prefix ``\texttt{griffin::}'' in front of a given function of class name or use the directive ``\mbox{\texttt{using namespace griffin}}''.
\end{minipage}}
\medskip
...
The following example computes SM predictions for the matrix element $M_{VV}$ for $e^+e^- \to d\bar{d}$ to electroweak NNLO and leading higher-order precision on the Z peak and NLO precision off-peak:
