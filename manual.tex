...
\paragraph*{Program release:} GRIFFIN version 1.0 (September xx, 2022).
\paragraph*{Language:} C++11 (2011 ISO C++), for example {\tt gcc 4.8.1} and beyond.
\paragraph*{Download:} \url{http://}
...
The theory framework is implemented within a structure of classes in C++. In {\sc v1.0}, only the SM predictions for EWPOs and polarized matrix elements near the Z-peak up to NNLO have been implemented. Yet in principle one can use the {\sc GRIFFIN} framework to implement predictions for other observables, different (BSM) models, and/or up to arbitrary higher orders. This section will document the structure of the classes by introducing each class and where to find and modify them in the code.
...
Couplings               & \begin{tabular}[c]{@{}l@{}}$\alpha(0)$\,\\ $\Delta\alpha\equiv 1-\alpha(0)/\alpha(M_\PZ^2)$\\ $ \alpha_s^{\overline{\text{MS}}}(M_\PZ^2)$,\, $G_{\mu}$\end{tabular} & \begin{tabular}[c]{@{}l@{}}(polarized) matrix element\\ square near Z-peak\end{tabular} & $\re{M_{ij}M_{kl}^{*}}$                                                                                                                
\end{tabular}
...
\begin{equation}
    \overline{M}=M^{\rm exp}/\sqrt{1+(\Gamma^{\rm exp}/M^{\rm exp})^2} \,\,,\overline{\Gamma}=M^{\rm exp}/\sqrt{1+(\Gamma^{\rm exp}/M^{\rm exp})^2}.
\label{massscheme}
\end{equation}
